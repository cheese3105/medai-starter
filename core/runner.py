...
class Runner:
    ...
    def run_episode(self, episode_input: EpisodeInput, *,
        ...
        base_context: dict[str, Any] = {
            ...
            "evidence": "", "loop_history": "",
            "ltm_facts": ltm_facts or "", "stm_history": stm_session_history or "",
        }
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            if self.has_retrieval:
                ...
                if self.logger.verbose:
                    self.logger.trace(qid, iteration, "retrieval", context["question"], context["choices"],
            ...
            if self.logger.verbose:
                self.logger.trace(qid, iteration, "reasoning", context["question"], context["choices"],
                                  out.data.get("prompt", ""), out.data.get("raw_response", ""),
                                  out.data, out.latency_ms, out.token_usage)

            # Verifier
            if self.has_verifier:
                context.update(draft_answer=draft_answer, draft_explanation=draft_explanation, draft_confidence=draft_confidence)
                ...
                if self.logger.verbose:
                    self.logger.trace(qid, iteration, "verifier", context["question"], context["choices"],
                                      out.data.get("prompt", ""), out.data.get("raw_response", ""),
                                      out.data, out.latency_ms, out.token_usage)
                ...
                # Query rewrite: dùng agent riêng nếu có, fallback về verifier.suggested_query
                if self.has_query_rewriter:
                    ...
                    new_query = rw_out.data.get("rewritten_query", context["question"])
                    ...
                    self.logger.stage_end(qid, iteration, "query_rewriter",
                                          f'query="{new_query[:60]}..."' if len(new_query) > 60 else f'query="{new_query}"',
                                          rw_out.latency_ms)
                    if self.logger.verbose:
                        self.logger.trace(qid, iteration, "query_rewriter", context["question"], context["choices"],
